...
const AiBoard = () => {
  ...
  const minimax = (newData: string[], depth: number, isMaximizing: boolean): number => {
    const winner = checkWin(newData);
    if (winner === "x") return -10 + depth;
    if (winner === "0") return 10 - depth;
    if (checkDraw(newData)) return 0;
  ...
  const findBestMove = (newData: string[]): number => {
    let bestScore = -Infinity;
    let move = -1;
    for (let i = 0; i < newData.length; i++) {
      if (newData[i] === "") {
        newData[i] = "0";
        const score = minimax(newData, 0, false);
        newData[i] = "";
        if (score > bestScore) {
          bestScore = score;
          move = i;
  ...
  const makeAiMove = (newData: string[]) => {
    const bestMove = findBestMove(newData);
    if (bestMove !== -1) {
      newData[bestMove] = "0";
      setData(newData);
      setCount(count + 1);
      ...
      {(win || draw) && (
        <div className="flex flex-col">
          {[
            ...
            { label: "Back to selection", onClick: () => navigate(ROUTES.aichoose) },
